...
namespace :pkg do
  ...
  task :generate_source do
    ...
    `git archive --prefix=foreman-#{version}/ #{ref} > pkg/foreman-#{version}.tar`
    `npm install`
    `tar --transform 's,^,foreman-#{version}/,' -r -f pkg/foreman-#{version}.tar node_modules`
    `bzip2 -9 pkg/foreman-#{version}.tar`
  end
end
